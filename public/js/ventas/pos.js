...
let currentAlmacen =
  typeof DEFAULT_ALMACEN !== "undefined" ? DEFAULT_ALMACEN : 0;
...
const activeMethodsContainer = document.getElementById("activePaymentMethods");
...
payButton.addEventListener("click", () => {
  ...
  const cashMethod = paymentMethodsDB.find((m) =>
...
window.renderActivePaymentMethods = function () {
  ...
  activePaymentMethods.forEach((methodId) => {
    ...
    div.className =
      "p-4 flex items-center justify-between gap-4 animate-in slide-in-from-top-2 duration-200";
    div.innerHTML = `
...
window.addPaymentMethod = function (methodId) {
  ...
  if (!activePaymentMethods.includes(methodIdStr)) {
    ...
    // Focus new input
    setTimeout(
      () => document.getElementById(`${methodIdStr}Input`).focus(),
      50,
    );
    calculateBalance();
  }
...
window.calculateBalance = function () {
  const totalToPay = parseFloat(modalTotalToPay.textContent.replace("S/ ", ""));
  ...
  const isOnlyCash =
    activePaymentMethods.length === 1 &&
    cashMethod &&
    activePaymentMethods[0] == cashMethod.id;
  ...
  if (balance >= 0) {
    ...
    balanceElem.parentElement.classList.replace("bg-orange-50", "bg-green-50");
  } else {
    ...
    balanceElem.parentElement.classList.replace("bg-green-50", "bg-orange-50");
  }
};
        ...
        .then((res) => {
          if (res.status === "success") {
            ...
            selectCustomer(
              ...
              client.nombres,
              client.id_tipo_documento,
              client.tipo_doc_nombre,
            );
            closeCustomerModal();
            alert(res.message);
          } else {
...
window.toggleMobileMenu = function () {
  ...
  const hamburgerBtn = document.querySelector(
    'button[onclick="toggleMobileMenu()"] .material-symbols-outlined',
  );
...
window.loadWarehouses = function () {
  fetch(BASE_URL + "/almacen/listar")
    ...
    .then((res) => {
      if (res.status === "success") {
        const selectors = [document.getElementById("posWarehouse")].filter(
          (el) => el !== null,
        );
  ...
  fetch(BASE_URL + "/categorias/listar")
    ...
    .then((data) => {
      ...
      data.forEach((cat) => {
        ...
        btn.className =
          "px-6 py-2 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 font-medium text-sm transition-colors category-btn";
...
window.loadProducts = function () {
  const url = `${BASE_URL}/productos/listar?almacen_id=${currentAlmacen}&categoria_id=${currentCategoria}&es_pos=1`;
  fetch(url)
    .then((res) => res.json())
    .then((res) => {
      if (res.status === "success") {
...
window.renderProducts = function () {
  ...
  filtered.forEach((p) => {
    const image = p.imagen_producto
      ? `${BASE_URL}/uploads/productos/${p.imagen_producto}`
      : `${BASE_URL}/uploads/productos/producto.png`;
    const stock = parseFloat(p.stock);
    const stockClass =
      stock <= 5 ? "bg-red-500" : "bg-slate-900/80 backdrop-blur-sm";

    const card = document.createElement("div");
    card.className =
      "product-card bg-white dark:bg-slate-900 rounded-xl p-3 border border-slate-200 dark:border-slate-800 shadow-sm hover:border-primary transition-all cursor-pointer group";
    ...
    card.onclick = () =>
      addProduct(p.id, p.nombre_producto, p.precio_venta, image);
    grid.appendChild(card);
  });
};

const searchInputs = [document.getElementById("productSearch")].filter(
  (el) => el !== null,
);
...
window.renderPaymentButtons = function () {
  ...
  paymentMethodsDB.forEach((method) => {
    ...
    btn.className =
      "flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-primary hover:text-primary transition-all text-xs font-bold";
    btn.innerHTML = `
...
window.processPayment = function () {
  ...
  const esFactura = selectedDocRadio
    ? selectedDocRadio.dataset.esFactura === "true"
    : false;
  const esBoleta = selectedDocRadio
    ? selectedDocRadio.dataset.esBoleta === "true"
    : false;
  const esClienteRUC = selectedCustomerTipoDocNombreVal
    .toUpperCase()
    .includes("RUC");
  const esClienteDNI = selectedCustomerTipoDocNombreVal
    .toUpperCase()
    .includes("DNI");
        ...
        .then((res) => {
          if (res.status === "success") {
            ...
            Swal.fire({
              ...
              confirmButtonColor: "#13ec49",
              confirmButtonText:
                '<span class="flex items-center gap-2"><span class="material-symbols-outlined">print</span> Imprimir Ticket</span>',
